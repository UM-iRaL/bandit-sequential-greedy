%% adversarial two robots vs. four targets battle.
clear all;
close all;
...
for rep = 1:num_rep
    ...
    v_tg = [0.35; 0.5; 0.72; 0.6]*20;
    yaw_tg = [deg2rad(0);deg2rad(90);deg2rad(90);deg2rad(90)];
    ...
    T(4) = adversarial_target_v1(4, v_tg(4), tg_true(:,4,1,rep), yaw_tg(4), run_len, 'circle', dT);
...
%                     reward(r, t, rep) = (curr_obj_BSG - prev_obj_BSG) / (prev_obj_BSG/2 - prev_obj_BSG);
        ...
        if viz
            ...
            if strcmp(planner_name, 'bsg')
                title('BSG: 2 Robots vs. 4 Adversarial Targets [2X]', 'FontSize', 15);
            else
                title('SG-Heuristic: 2 Robots vs. 4 Adversarial Targets [2X]', 'FontSize', 15);
            end
