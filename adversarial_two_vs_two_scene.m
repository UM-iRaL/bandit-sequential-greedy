%% adversarial two robots vs. two target battle.
clear all;
%close all;
...
% Visibility map
vis_map = init_blank_ndmap([-map_size*6; -map_size*6],[map_size*6; map_size*6],0.25,'logical');
...
for rep = 1:num_rep
    ...
    for r = 1:num_robot
        if r == 1
            R(r) = robot_nx(x_true(1, r, :, rep), 150, deg2rad(74));
        else
            R(r) = robot_nx(x_true(1, r, :, rep), 100, deg2rad(74));
        end
    ...
    T(1) = adversarial_target_v1(1, v_tg(1), tg_true(:,1,1,rep), yaw_tg(1), run_len, 'straight');   %left
    T(2) = adversarial_target_v1(2, v_tg(2), tg_true(:,2,1,rep), yaw_tg(1), run_len, 'straight');   %right
...
%             h0.tg(kk) = draw_pose_nx([], tg_true(:,kk,1,rep),'g',5);
    ...
    for t = 1:run_len
        ...
        for kk = 1:num_tg
            ...
            tg_true(:, kk, t+1, rep) = T(kk).get_position(t+1)';
        end
...
%         end
        ...
        for kk = 1:num_tg
            if ~detected(kk)
                cov_z = [R(r).r_sigma 0; 0 R(r).b_sigma];
                %estm_tg_cov( :, :, kk) = inv_rb_cov([0;0;0], [400*sqrt(2) 3], zeros(3,3), cov_z);
        ...
        if strcmp(planner_name, 'bsg')
            ...
            for r =  num_robot:-1:1% % 1:num_robot%itr_order%
                if size(estm_tg_save{t, rep}, 2) ~= 0
                    ...
                    prev_obj_BSG = objective_function(prev_robot_states, [estm_tg], prev_r_senses, prev_fovs);
                    ...
                    curr_obj_BSG = objective_function(prev_robot_states, [estm_tg], prev_r_senses, prev_fovs);
